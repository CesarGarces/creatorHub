...
const navItems: NavItem[] = [
  ...
  { label: "Agents", href: "/agents", icon: <AgentIcon />, section: "WORKSPACE" },
  { label: "Tools", href: "/tools", icon: <ToolIcon /> },
  { label: "Assets", href: "/assets", icon: <AssetIcon /> },
  { label: "History", href: "/history", icon: <HistoryIcon /> },
  { label: "Credits", href: "/credits", icon: <CreditIcon />, section: "ACCOUNT" },
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3 space-y-1">
        {groupedItems.map((group, gi) => (
          <div key={gi}>
            {group.section && (
              <div className={cn("text-[10px] font-semibold uppercase tracking-wider text-text-dim mb-2", collapsed ? "px-2" : "px-3")}>
      ...
      <div className="border-t border-border p-3 space-y-3">
        {!collapsed && (
          <CreditDisplay balance={balance} size="sm" className="w-full justify-center" />
        )}
        <Separator />
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-text truncate">{user?.email?.split("@")[0]}</p>
...
function DashboardIcon() {
  return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="9" rx="1"/><rect x="14" y="3" width="7" height="5" rx="1"/><rect x="14" y="12" width="7" height="9" rx="1"/><rect x="3" y="16" width="7" height="5" rx="1"/></svg>;
}

function AgentIcon() {
  return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/></svg>;
}
...
function HistoryIcon() {
  return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M12 7v5l4 2"/></svg>;
}
